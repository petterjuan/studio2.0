...
function isFirestoreAuthError(error: unknown): boolean {
  ...
  return e.code === 7 || e.code === 'UNAUTHENTICATED' || (typeof e.message === 'string' && e.message.includes('Could not refresh access token'));
}

// Firestore snapshot -> Article
...
async function fetchArticlesFromFirestore(count?: number): Promise<Article[] | null> {
  try {
    let query = firestore.collection('articles').orderBy('publishedAt', 'desc');
    ...
    if (isFirestoreAuthError(error)) {
      console.warn('// Firestore API not available. Using fallback data.');
      return null;
    }
    throw error;
  }
...
export async function getArticles(first?: number): Promise<Article[]> {
  ...
  if (!articlesFromDb) return first ? fallbackArticles.slice(0, first) : fallbackArticles;

  const uniqueArticles = Array.from(new Map(articlesFromDb.map(a => [a.handle || a.id, a])).values());
  return first ? uniqueArticles.slice(0, first) : uniqueArticles;
}

// Get single article by handle
export async function getArticleByHandle(handle: string): Promise<Article | null> {
  try {
    const snapshot = await firestore.collection('articles').where('handle', '==', handle).limit(1).get();
    if (snapshot.empty) return getFallbackArticles().find(a => a.handle === handle) ?? null;

    return docToArticle(snapshot.docs[0]);
  } catch (error) {
    if (isFirestoreAuthError(error)) return getFallbackArticles().find(a => a.handle === handle) ?? null;
    throw error;
  }
}
